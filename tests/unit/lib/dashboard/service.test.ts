...
import { buildLocalDashboardAnalytics } from "@/lib/dashboard/service"
...
describe("local dashboard provider parity", () => {
  ...
  it("uses an available accounting purchase valuation for a legacy sale line", () => {
    const data = {
      weapons: [{
        id: "W-LEGACY", status: "Sold", weaponType: "Pistol", brand: "Legacy", model: "Nine",
        dateAdded: "2026-07-01", purchasePrice: 420,
        purchasePriceValuation: { accountingAmount: 420, accountingCurrency: "USD" },
      }],
      accessories: [], ammunition: [], shipments: [], payments: [], customers: [], suppliers: [], auditLogs: [], notifications: [], users: [], savedFilters: [],
      settings: { currencyCode: "USD", accountingCurrencyCode: "USD" },
      invoices: [{
        id: "I-LEGACY", type: "Sale", voided: false, date: "2026-08-10", dueDate: "2026-08-10",
        totalNegotiated: 700, totalNegotiatedAccounting: 700, balance: 0,
        lineItems: [{ itemType: "weapon", itemId: "W-LEGACY", name: "Legacy Nine", quantity: 1, unitPrice: 700, total: 700 }],
      }],
    } as unknown as AllData

    const analytics = buildLocalDashboardAnalytics(data, { start: "2026-08-01", end: "2026-08-31" })
    expect(analytics.current).toMatchObject({ revenue: 700, cost: 420, profit: 280, marginPct: 40, costCoveragePct: 100 })
    expect(analytics.products[0]).toMatchObject({ key: "weapon:W-LEGACY", cost: 420, profit: 280 })
  })
  ...
  it("returns the complete inventory list for aging dialogs", () => {
    const data = {
      weapons: [],
      accessories: Array.from({ length: 13 }, (_, index) => ({
        id: `A${index + 1}`, name: `Accessory ${index + 1}`, type: "Accessory", quantity: 1,
        safetyThreshold: 0, dateAdded: "2025-01-01", priceValuation: { accountingAmount: 10 },
      })),
      ammunition: [], shipments: [], invoices: [], payments: [], customers: [], suppliers: [], auditLogs: [], notifications: [], users: [], savedFilters: [],
      settings: { accountingCurrencyCode: "USD" },
    } as unknown as AllData

    const analytics = buildLocalDashboardAnalytics(data, { start: "2026-08-01", end: "2026-08-31" })
    expect(analytics.inventory.items).toHaveLength(13)
    expect(analytics.inventory.deadStock).toBe(13)
  })
})
