...
describe("display currency user preference", () => {
  ...
  it("starts a user without saved preferences in USD regardless of the store preference", async () => {
    const upsert = vi.spyOn(db, "dbUpsertUserPreferences").mockResolvedValue()

    const result = await useStore.getState().updateUserPreferences({ reportViewMode: "accounting" })
    ...
    expect(upsert).toHaveBeenCalledWith(expect.objectContaining({ displayCurrency: "USD" }))
  })

  it("preserves an existing per-user display currency", async () => {
    useStore.setState({
      userPreferences: { userId: "U001", displayCurrency: "SAR", reportViewMode: "display" },
    })
    const upsert = vi.spyOn(db, "dbUpsertUserPreferences").mockResolvedValue()
    ...
    expect(result).toEqual({ success: true })
    expect(useStore.getState().userPreferences?.displayCurrency).toBe("SAR")
    expect(upsert).toHaveBeenCalledWith(expect.objectContaining({ displayCurrency: "SAR" }))
  })
