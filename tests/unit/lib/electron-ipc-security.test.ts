...
describe("Electron storage IPC boundary", () => {
  ...
  it("keeps local password recovery behind validated, role-checked IPC methods", () => {
    const preload = source("electron/preload.cts")
    const storage = source("electron/ipc/storage-handler.ts")
    const recovery = source("electron/services/password-recovery-service.ts")
    expect(preload).toContain('ipcRenderer.invoke("password-recovery:request", input)')
    expect(preload).toContain('ipcRenderer.invoke("password-recovery:approve", input)')
    expect(storage).toContain("recoveryCompleteSchema.parse(input)")
    expect(storage).toContain("requireSqliteSelected()")
    expect(recovery).toContain("requireLocalSession()")
    expect(recovery).toContain('actor.role !== "Admin"')
    expect(recovery).toContain("timingSafeEqual(actual, expected)")
    expect(recovery).toContain("attempts=attempts+1")
    expect(recovery).toContain("MAX_REQUESTS_PER_HOUR = 5")
  })
})
