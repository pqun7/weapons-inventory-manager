...
describe("schema-flexible spreadsheet extraction", () => {
  ...
  it("preserves spreadsheet hidden-state and merged ranges in the normalized model", () => {
    const workbook: XLSX.WorkBook = { SheetNames: ["Items"], Sheets: { Items: { A1: { t: "s", v: "Product" }, B1: { t: "s", v: "Qty" }, "!ref": "A1:B1", "!merges": [{ s: { r: 0, c: 0 }, e: { r: 0, c: 1 } }] } }, Workbook: { Sheets: [{ Hidden: 1 }] } }
