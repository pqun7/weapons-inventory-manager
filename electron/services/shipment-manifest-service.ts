...
function persistItems(importId: string, items: ParsedManifestItem[]): void {
  ...
  const insert = db.prepare(`INSERT INTO shipment_import_items (
    ...
    serial_number, serial_numbers_json, quantity, unit_price, total_price, currency, country_of_origin,
  ...
  for (const item of items) insert.run(
    ...
    item.unitPrice, item.totalPrice, item.currency, item.countryOfOrigin, item.weaponTypeId, item.weaponSubtypeId, item.brandId,
...
export function getManifestReview(importId: string): ShipmentManifestReview {
  ...
  return {
    ...
    origin: cleanNullable(row.origin), destination: cleanNullable(row.destination), currency: cleanNullable(row.currency), reviewNote: cleanNullable(row.review_note), aiProvider: cleanNullable(row.ai_provider), aiModel: cleanNullable(row.ai_model),
...
export function updateManifestDetails(importId: string, patch: ManifestDetailsPatch, user: CurrentUser): ShipmentManifestReview {
  ...
  const columns: Record<keyof ManifestDetailsPatch, string> = {
    ...
    origin: "origin", destination: "destination", currency: "currency", reviewNote: "review_note",
  }
  getDb().transaction(() => {
    const existing = getDb().prepare("SELECT * FROM shipment_imports WHERE id = ?").get(importId) as Record<string, unknown>
    const changes: string[] = []
    for (const [field, value] of Object.entries(patch) as Array<[keyof ManifestDetailsPatch, string | null | undefined]>) {
      const column = columns[field]
      if (!column) continue
      const stored = field === "currency" && value ? value.trim().toUpperCase() : cleanNullable(value)
    ...
    if (item.productType === "weapon") {
      for (const serial of item.serialNumbers) {
        ...
        weapons.push({
          ...
          condition: "Excellent", status: "Available", purchasePrice: purchaseValuation.originalAmount, retailPrice: 0, wholesalePrice: 0, actualFinalPrice: null,
