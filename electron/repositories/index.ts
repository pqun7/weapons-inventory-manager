...
import { listShipmentCosts, mapInventoryCostSnapshotRow, mapProductCostRow } from "../services/product-cost-service.js"
...
export class AppRepository {
  getAll(): AllData {
    ...
    const snapshotsByProduct = new Map(
      (db.prepare("SELECT * FROM inventory_cost_snapshots").all() as Record<string, unknown>[])
        .map(mapInventoryCostSnapshotRow)
        .map((snapshot) => [`${snapshot.productType}:${snapshot.productId}`, snapshot] as const),
    )
    const costsByProduct = new Map<string, ReturnType<typeof mapProductCostRow>[]>()
    for (const row of db.prepare("SELECT * FROM product_costs ORDER BY created_at, id").all() as Record<string, unknown>[]) {
      const cost = mapProductCostRow(row)
    ...
    `).all() as Record<string, unknown>[])
      ...
      .map((item) => ({ ...item, costSnapshot: snapshotsByProduct.get(`weapon:${item.id}`), additionalCosts: costsByProduct.get(`weapon:${item.id}`) ?? [] }))
    ...
    const inventoryProductTypes = (db.prepare("SELECT id, category, name FROM inventory_product_types ORDER BY category, name COLLATE NOCASE").all() as Record<string, unknown>[])
    ...
    return { weapons, accessories, ammunition, shipments, invoices, payments, customers, suppliers, auditLogs, notifications, users, settings, savedFilters, inventoryProductTypes }
  }
  ...
  upsertUserPreferences(p: UserPreferences): void {
    ...
    db.prepare(`INSERT INTO user_preferences (user_id, display_currency, report_view_mode, language, date_format, inventory_visible_columns, created_at, updated_at)
      VALUES (@user_id, @display_currency, @report_view_mode, @language, @date_format, @inventory_visible_columns, datetime('now'), datetime('now'))
      ON CONFLICT(user_id) DO UPDATE SET
        display_currency = @display_currency,
        report_view_mode = @report_view_mode,
        language = @language,
        date_format = @date_format,
        inventory_visible_columns = @inventory_visible_columns,
  ...
  insertAccessory(a: Accessory): void {
    ...
    db.prepare(`INSERT INTO accessories (id, name, type, quantity, safety_threshold, price, price_currency, price_valuation,
      retail_price, wholesale_price, retail_price_valuation, wholesale_price_valuation, retail_price_mode, wholesale_price_mode,
      date_added, warehouse, shelf, bin)
      VALUES (@id, @name, @type, @quantity, @safety_threshold, @price, @price_currency, @price_valuation,
      @retail_price, @wholesale_price, @retail_price_valuation, @wholesale_price_valuation, @retail_price_mode, @wholesale_price_mode,
      @date_added, @warehouse, @shelf, @bin)`).run(row)
  }
  ...
  updateAccessory(a: Accessory): void {
    ...
    db.prepare(`UPDATE accessories SET name = @name, type = @type, quantity = @quantity, safety_threshold = @safety_threshold,
      ...
      retail_price_valuation = @retail_price_valuation, wholesale_price_valuation = @wholesale_price_valuation,
      retail_price_mode = @retail_price_mode, wholesale_price_mode = @wholesale_price_mode,
  ...
  insertAmmunition(a: Ammunition): void {
    ...
    db.prepare(`INSERT INTO ammunition (id, name, caliber, package_type, units_per_package, full_packages, loose_rounds,
      safety_threshold, price, price_currency, price_valuation, retail_price, wholesale_price, retail_price_valuation,
      wholesale_price_valuation, retail_price_mode, wholesale_price_mode, date_added, warehouse, shelf, bin)
      VALUES (@id, @name, @caliber, @package_type, @units_per_package, @full_packages, @loose_rounds,
      @safety_threshold, @price, @price_currency, @price_valuation, @retail_price, @wholesale_price, @retail_price_valuation,
      @wholesale_price_valuation, @retail_price_mode, @wholesale_price_mode, @date_added, @warehouse, @shelf, @bin)`).run({ ...row, name: a.name ?? a.caliber })
  }
  ...
  updateAmmunition(a: Ammunition): void {
    ...
    db.prepare(`UPDATE ammunition SET caliber = @caliber, package_type = @package_type, units_per_package = @units_per_package,
      ...
      name = @name, price = @price, price_currency = @price_currency, price_valuation = @price_valuation,
      retail_price = @retail_price, wholesale_price = @wholesale_price,
      retail_price_valuation = @retail_price_valuation, wholesale_price_valuation = @wholesale_price_valuation,
      retail_price_mode = @retail_price_mode, wholesale_price_mode = @wholesale_price_mode,
      warehouse = @warehouse, shelf = @shelf, bin = @bin WHERE id = @id`).run({ ...row, name: a.name ?? a.caliber })
  }

  insertCustomer(c: Customer): void {
    const db = getDb()
    const row = mappers.customerToRow(c)
    db.prepare(`INSERT INTO customers (id, name, phone, email, address, is_wholesale_buyer, wholesale_discount_percent, notes, custom_fields, date_added)
      VALUES (@id, @name, @phone, @email, @address, @is_wholesale_buyer, @wholesale_discount_percent, @notes, @custom_fields, @date_added)`).run(row)
  }
  ...
  getOverrides(): ExchangeRateOverrideRow[] {
    return getDb().prepare("SELECT * FROM exchange_rate_overrides ORDER BY currency_code").all() as unknown as ExchangeRateOverrideRow[]
  }
